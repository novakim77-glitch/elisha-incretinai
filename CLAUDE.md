...
> 마지막 업데이트: 2026-05-17 (세션 3 — CLAUDE.md 단일 통합 + 문서 인덱스 추가)
...
### 앱·봇·미니앱 소스 코드
```
IncretinAi_v7.0_Adaptive.html              ← 배포 PWA (메인)
...
**우선순위 높음 (다음 세션에서 진행)**
- 봇→앱 동기화 신뢰성 Phase A 진행 — 연결 상태 표시등 + Pull-to-Refresh
...
**미니 앱 개발 (다음 세션 — Vite 셋업부터)**
- Step 1: Vite 프로젝트 생성 + `Synca_MiniApp_Prototype.jsx` 이식 (30분)
  - `npm create vite@latest synca-miniapp -- --template react`
  - 로컬 `npm run dev` 실행 확인
- Step 2: Telegram WebApp SDK 연결 — `Telegram.WebApp.initDataUnsafe.user.id` 인증 (1시간)
- Step 3: Firestore 연결 — 더미 데이터 → 실제 사용자 데이터 읽기 (2~3시간)
- Step 4: GitHub Pages 배포 — `npm run build` + gh-pages (30분)
- Step 5: 봇 InlineKeyboard에 미니 앱 버튼 추가 (30분)
- 미니 앱 콘텐츠 β_seq 나머지 10종 제작 (집밥 3종 추가, 외식 3종 추가, 식후 산책 4종)

**우선순위 중간**
- `/broadcast` Unknown command 현상 원인 파악 및 수정
- ADMIN_CHAT_ID Fly.io secrets 설정 완료 확인
- 챌린지 참가자 /link 연결 + 시작 체중(sw) 설정 가이드 발송
- 봇 v2.0: /preload 명령어 추가 (β_pre 로깅)
- TIPS 신청 준비
