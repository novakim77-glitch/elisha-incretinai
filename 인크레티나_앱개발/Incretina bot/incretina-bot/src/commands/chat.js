...
function buildMealFeedback(meal, allMeals, profile) {
  ...
  if (mealType === 'lateNight') {
    lines.push('19시 이후 식사 — 늦은 식사는 대사 리듬에 불리해요. 내일 아침 단식 1시간 연장으로 회복 가능!');
  }
  ...
  if (typeof meal.betaScore === 'number' && meal.betaScore < 0.5) {
    lines.push('식사 순서(채소→단백질→탄수) 개선으로 인크레틴 반응을 높일 수 있어요.');
  }
...
\`\`\`
betaScore는 0-1 범위로 식사 순서/섬유 품질(채소·단백질 우선, 정제탄수 적음=높음).
